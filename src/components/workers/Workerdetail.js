...
const WorkerDetail = ({match})=>{
  ...
  const invite =  (no) => {
    ...
    const sms_url = `http://4e3a861a301f.ngrok.io/sendsms?body=${body}&number=${no}`
    const response  =  axios.get(sms_url)
    console.log(response.data)

    const call_url = `http://4e3a861a301f.ngrok.io/makeACall?body=Hellow,${msg}&number=${no}&name=${name}`
